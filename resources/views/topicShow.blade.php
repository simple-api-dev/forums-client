                ...
                <main class="flex flex-col w-full bg-white overflow-y-auto overflow-x-auto m-10">
                    ...
                    <div>
                        <form class="bg-slate-100 p-5" method="POST"
                              action="{{getenv('FORUM_CLIENT')}}/storeCommentPost">
                            @csrf
                    ...
                    <div>
                        @foreach($topic_content->comments  as $key => $value)
                            <div class="m-5 p-5">
                                ...
                                [{{$value->status}}]
                                {{$value->author_id}}
                                <a href="{{getenv('FORUM_CLIENT')}}/deleteComment/{{$forum_id}}/{{$forum_slug}}/{{$topic_id}}/{{$topic_slug}}/{{$value->id}}?apikey={{getenv('API_KEY')}}"><i
